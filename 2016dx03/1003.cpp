...
{
	int x = -1;
	if(hefa_knight(i - 2, j - 1) && ~a_knight[i - 2][j - 1]) x = a_knight[i - 2][j - 1];
	if(hefa_knight(i - 1, j - 2) && ~a_knight[i - 1][j - 2])
	{
		if(~x) x &= a_knight[i - 1][j - 2];
		else x = a_knight[i - 1][j - 2];
	}
	if(~x) a_knight[i][j] = 1 ^ x;
}
void init_knight()
{
	clr(a_knight, -1);
...
int main()
{
	...
	FORR(i,10){FORR(j,10)printf("%d ",a_knight[i][j]);puts("");}puts("");
